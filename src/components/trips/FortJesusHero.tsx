...
export function FortJesusHero({ trip }: { trip: Trip }) {
  ...
  return (
    <div className="relative h-[65vh] sm:h-[75vh] min-h-[500px] overflow-hidden group">
      {/* Slides Background */}
      {slides.map((slide, idx) => (
        ...
        >
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-neutral-950 via-neutral-900/60 to-black/20" />
          <div className="absolute inset-0 bg-black/20" /> {/* Slight uniform darkening */}
        </div>
      ))}

      {isComingSoon && (
        <div className="absolute top-6 right-6 z-20 bg-amber-500 text-white px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider">
          Coming Soon
        </div>
      )}

      {/* Persistent Back Link at Top */}
      ...
      {/* Content Area */}
      <Container className="absolute bottom-0 left-0 right-0 pb-12 sm:pb-16 z-20 pointer-events-none">
        
        {/* Main Title appearing only on the first slide or consistently? Let's show consistent tags, and specific slide titles */}
        <div className="pointer-events-auto max-w-3xl">
          <div className="flex items-center gap-3 mb-4">
            <span className="px-3 py-1 bg-teal-500 text-white text-xs font-black uppercase tracking-wider rounded-full shadow-lg">
              {trip.category}
            </span>
            {trip.rating && (
              <div className="flex items-center gap-1.5 bg-neutral-900/60 sm:bg-white/20 sm:backdrop-blur-md px-3 py-1 rounded-full shadow-lg">
              ...
              return (
                ...
                >
                  ...
                  <Heading
                    level={idx === 0 ? "h1" : "h2"}
                    className={cn(
                      "text-white mb-3 !font-bold drop-shadow-lg",
                      idx === 0 ? "text-3xl sm:text-4xl md:text-5xl lg:text-6xl" : "text-2xl sm:text-3xl md:text-4xl"
                    )}
                  >
                    {slide.title}
                  </Heading>
                  <p className="text-white/90 text-base sm:text-lg max-w-2xl drop-shadow-md leading-relaxed">
                    {slide.description}
                  </p>
                </div>
              );
      ...
      <div className="absolute bottom-4 sm:bottom-6 left-0 right-0 z-30 pointer-events-none">
        <Container>
          <div className="flex justify-start sm:justify-end gap-2 pointer-events-auto">
            {slides.map((_, idx) => (
              <button
                key={`dot-${idx}`}
                ...
                <div
                  className={cn(
                    ...
                    currentSlide === idx
                      ...
                      : "w-2 bg-white/50 group-hover:bg-white border border-transparent shadow-sm"
                  )}
                />
              </button>
            ))}
