...
const galleryItems = [
    {
        ...
        gridClassName: "md:col-span-2 md:row-span-2", // Large feature
    },
    {
            ...
            <Container>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 md:gap-4 md:auto-rows-[10.5rem]">
                    {galleryItems.map((item, idx) => (
                        <div
                            key={idx}
                            className={`relative overflow-hidden rounded-2xl bg-neutral-900 group h-56 sm:h-52 md:h-auto ${item.gridClassName}`}
                        >
                            <Image
                                src={item.src}
                                alt={item.alt}
